...
function payuBase(env: string): string {
  return env === 'test' ? 'https://test.payu.in' : 'https://secure.payu.in';
}
...
export const payuAdapter: GatewayAdapter = {
  ...
  async createOrder(creds, ctx: OrderContext): Promise<OrderResult> {
    ...
    const txnId = `PU_${ctx.paymentId.replace(/-/g, '').slice(0, 16)}_${Date.now()}`;
    ...
    return {
      orderId: txnId,
      publicKey: merchantKey,
      checkoutUrl: `${payuBase(env === 'test' ? 'test' : 'production')}/_payment`,
      checkoutParams: {
        key: merchantKey,
        txnid: txnId,
        ...
        surl: `${ctx.webhookBaseUrl}/functions/v1/payment-webhook?gateway=payu`,
        furl: `${ctx.webhookBaseUrl}/functions/v1/payment-webhook?gateway=payu`,
        hash,
        udf1: ctx.paymentId,
      },
    };
  ...
  async testConnection(creds) {
    ...
    return { ok: true, message: 'PayU credentials saved' };
  },
};
