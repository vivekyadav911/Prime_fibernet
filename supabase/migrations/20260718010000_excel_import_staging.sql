-- Diff-based Excel bulk import: staging, audit history, plans.name unique, commit RPC

-- ---------------------------------------------------------------------------
-- Match-key uniqueness for plans (users.email and payments.payment_number already unique;
-- officers.employee_id already has a partial unique index)
-- ---------------------------------------------------------------------------
CREATE UNIQUE INDEX IF NOT EXISTS idx_plans_name_active
  ON public.plans (name)
  WHERE COALESCE(is_deleted, false) = false;

-- ---------------------------------------------------------------------------
-- Staging: parsed rows held until admin confirms
-- ---------------------------------------------------------------------------
CREATE TABLE IF NOT EXISTS public.import_staging (
  id uuid PRIMARY KEY DEFAULT gen_random_uuid(),
  import_batch_id uuid NOT NULL,
  entity_type text NOT NULL
    CHECK (entity_type IN ('users', 'plans', 'officers', 'transactions')),
  row_number int NOT NULL,
  raw_row jsonb NOT NULL,
  match_key text,
  action text
    CHECK (action IN ('insert', 'update', 'unchanged', 'error')),
  diff jsonb,
  error_message text,
...
CREATE INDEX IF NOT EXISTS idx_import_staging_batch
  ON public.import_staging (import_batch_id);

CREATE INDEX IF NOT EXISTS idx_import_staging_batch_action
  ON public.import_staging (import_batch_id, action);

-- ---------------------------------------------------------------------------
-- Permanent audit trail of committed imports
-- ---------------------------------------------------------------------------
...
-- ---------------------------------------------------------------------------
-- RLS: admins can read; writes via service role / SECURITY DEFINER only
-- ---------------------------------------------------------------------------
ALTER TABLE public.import_staging ENABLE ROW LEVEL SECURITY;
ALTER TABLE public.import_history ENABLE ROW LEVEL SECURITY;

DROP POLICY IF EXISTS import_staging_admin_select ON public.import_staging;
CREATE POLICY import_staging_admin_select
  ON public.import_staging
  FOR SELECT
  USING (public.is_admin_user());

DROP POLICY IF EXISTS import_history_admin_select ON public.import_history;
CREATE POLICY import_history_admin_select
  ON public.import_history
  FOR SELECT
  USING (public.is_admin_user());

-- ---------------------------------------------------------------------------
-- commit_import_batch — applies staged rows in one transaction
-- Only updates columns present in each row's `diff` (field-level).
-- ---------------------------------------------------------------------------
...
BEGIN
  ...
  LOOP
    ...
    ELSIF v_entity = 'transactions' THEN
      -- Financial fields (amount/total_amount/status) must never reach here for confirmed rows —
      -- the edge function rejects those as error rows before staging as update.
  ...
  -- Drop staging rows for this batch (history retains the audit)
