-- Block second check-in after today's shift is completed; cancel stale pending requests.
...
BEGIN
  ...
  IF p_action = 'approve' AND v_req.type IN ('out_of_zone_checkin', 'late_checkin', 'second_shift_checkin') THEN
    SELECT * INTO v_shift
    FROM public.shifts
    ...
    IF FOUND AND (v_shift.check_out_time IS NOT NULL OR v_shift.status = 'completed') THEN
      RAISE EXCEPTION 'Maximum one attendance is allowed per day. Edit the existing attendance record instead.';
    END IF;
  END IF;
  ...
  INSERT INTO public.attendance_approval_audit_log (
    approval_request_id, action, performed_by, performed_by_name, notes
  ) VALUES (
    p_request_id, p_action, v_user_id, COALESCE(v_reviewer_name, 'Admin'), p_review_notes
  );
  ...
  IF p_action = 'reject' THEN
    ...
    PERFORM public.notify_collection_officer(
      v_req.officer_id, 'system', v_title, v_body,
      jsonb_build_object(
  ...
  IF v_req.type IN ('out_of_zone_checkin', 'late_checkin') THEN
    ...
    WHERE officer_id = v_req.officer_id AND shift_date = v_req.attendance_date AND status = 'active' LIMIT 1;
    ...
    ELSE
      INSERT INTO public.shifts (
        officer_id, shift_date, check_in_time, status, attendance_status, location,
        geofence_id, check_in_method, check_in_distance_m, approval_request_id
      ) VALUES (
        v_req.officer_id, v_req.attendance_date, COALESCE(v_req.requested_at, v_now),
        'active', 'present', v_point, v_req.geofence_id, 'approved_outside',
        v_req.distance_from_fence, p_request_id
      );
    END IF;

    PERFORM public.notify_collection_officer(
      v_req.officer_id, 'system',
      'Shift start approved',
      'Your out-of-zone check-in was approved. You are now clocked in.',
      jsonb_build_object('type', 'approval_approved', 'approval_request_id', p_request_id, 'approval_type', v_req.type, 'category', 'hr')
    );

  ELSIF v_req.type IN ('out_of_zone_checkout', 'early_checkout', 'missed_checkout') THEN
    SELECT * INTO v_shift FROM public.shifts
    WHERE officer_id = v_req.officer_id AND shift_date = v_req.attendance_date AND status = 'active' LIMIT 1;
    IF NOT FOUND THEN RAISE EXCEPTION 'No active shift found for approved check-out'; END IF;
    ...
    UPDATE public.shifts SET
      ...
      working_hours = ROUND(EXTRACT(EPOCH FROM (COALESCE(v_req.requested_at, v_now) - v_shift.check_in_time)) / 3600.0, 2),
      status = 'completed',
      attendance_status = 'present',
      approval_request_id = p_request_id
    WHERE id = v_shift.id;
  ...
  ELSIF v_req.type = 'manual_correction' THEN
    SELECT * INTO v_shift FROM public.shifts
    WHERE officer_id = v_req.officer_id AND shift_date = v_req.attendance_date
    ...
    ELSE
      INSERT INTO public.shifts (
        officer_id, shift_date, check_in_time, status, attendance_status, location,
        geofence_id, check_in_method, check_in_distance_m, approval_request_id, notes
      ) VALUES (
        v_req.officer_id, v_req.attendance_date, COALESCE(v_req.requested_at, v_now),
        'active', 'present', v_point, v_req.geofence_id, 'admin_override',
        v_req.distance_from_fence, p_request_id, v_req.reason
      );
    END IF;
  END IF;
...
UPDATE public.attendance_approval_requests aar
SET status = 'cancelled'
WHERE aar.status = 'pending'
  AND aar.type IN ('out_of_zone_checkin', 'late_checkin', 'second_shift_checkin')
  AND EXISTS (
    SELECT 1 FROM public.shifts s
    WHERE s.officer_id = aar.officer_id
      AND s.shift_date = aar.attendance_date
      AND (s.check_out_time IS NOT NULL OR s.status = 'completed')
  );
